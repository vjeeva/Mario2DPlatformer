/*
 BigCollision.Java (for Super Mario: The Zakaryan Strikes Back)
...
public class BigCollision {
	...
	public BigCollision(Rectangle[] mapInput) {
	...
	public void checkBelow() {
		...
		for (int i = 0; i <= mapRecArray.length - 1; i++) { // a for loop
			...
			if (userSprite.x + 12 >= mapRecArray[i].getX()
					&& userSprite.x <= mapRecArray[i].getX()
							+ mapRecArray[i].getWidth()) { // If the big user is
															// colliding with
															// the X position of
															// the map Rec...
				if (userSprite.y + 26 <= mapRecArray[i].getY()) { // If the big
																	// user is
																	// colliding
																	// below the
																	// mapRecArray...
				...
				if (userSprite.y >= mapRecArray[i].getY()
						+ mapRecArray[i].getHeight()) { // If the big user is
														// colliding above the
														// mapRecArray...
		...
		if (done1) { // If done1 is true...
			for (int i = 0; i <= mapRecArray.length - 1; i++) { // a for loop
				if (lowerArray[i].getY() - (userSprite.y + 26) < recLower
						.getY() - (userSprite.y + 26)) { // If the big user is
															// colliding below
															// the Rec...
		...
		if (done2) { // If done2 is true...
			for (int i = 0; i <= mapRecArray.length - 1; i++) { // a for loop
				if (Math.abs(userSprite.y + 26 - higherArray[i].getY()) < Math
						.abs(userSprite.y + 26 - recHigher.getY())) { // If the
																		// big
																		// user
																		// is
																		// colliding
		...
		if (done3) { // If done3 is true...
			if (userSprite.y + 26 < recLower.getY()) { // If the big user's Y
														// position is below the
														// recLower...
				userSprite.y += 10; // Stops the big user's jump
				if (!userSprite.invincible)
					EnemyCollision.bigCheckBelow(); // Checks below for enemy
												// collision
				CoinCollision.bigCollision(); // Checks for collision with coins
			}
			if (userSprite.y + 26 >= recLower.getY()) { // If the big user's Y
														// position is above the
														// recLower...
				userSprite.y = (int) (recLower.getY() - 26); // The big user's Y
																// position is
																// set to
																// recLower
																// subtract 26
	...
	public void checkRight() {
		...
		if (!userSprite.middle) {
			for (int i = 0; i <= mapRecArray.length - 1; i++) {
				if (userSprite.x + 12 >= mapRecArray[i].getX()
						&& userSprite.x + 12 <= mapRecArray[i].getX()
								+ mapRecArray[i].getWidth()) {
					if (mapRecArray[i] != lowerArray[i]
							&& mapRecArray[i] != higherArray[i])
						userSprite.x = (int) (mapRecArray[i].getX() - 12);
				}
			}
		}
		...
		if (userSprite.middle) { // If middle is true...
			for (int i = 0; i <= mapRecArray.length - 1; i++) { // a for loop
				if (userSprite.x + 12 >= mapRecArray[i].getX()
						&& userSprite.x + 12 <= mapRecArray[i].getX()
								+ mapRecArray[i].getWidth()) { // If the big
																// user is
																// colliding
																// with the X
																// position of
	...
	public boolean checkAbove() {
		...
		for (int i = 0; i < mapRecArray.length - 1; i++) { // a for loop
			if (userSprite.x <= mapRecArray[i].getX()
					+ mapRecArray[i].getWidth()
					&& userSprite.x >= mapRecArray[i].getX()) { // If the big
																// user is
																// colliding
																// with the X
																// position of
				...
				if (userSprite.y - 10 >= mapRecArray[i].getY()
						&& userSprite.y - 10 <= mapRecArray[i].getY()
								+ mapRecArray[i].getHeight()) { // If the big
																// user is
																// colliding
																// with the Y
																// position of
